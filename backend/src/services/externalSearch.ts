...
// 模拟维基百科搜索结果 (用于网络不通时)
function getMockWikipediaResults(query: string, limit: number): ExternalSearchResult[] {
  const mockData: Record<string, ExternalSearchResult[]> = {
    '机器学习': [
      {
        source: 'wikipedia',
        title: '机器学习',
...
// 维基百科搜索
export async function searchWikipedia(query: string, limit: number = 5): Promise<ExternalSearchResult[]> {
  try {
...
// arXiv 学术论文搜索
export async function searchArxiv(query: string, limit: number = 5): Promise<ExternalSearchResult[]> {
  try {
...
// GitHub 搜索（技术文档/代码）
export async function searchGitHub(query: string, limit: number = 5): Promise<ExternalSearchResult[]> {
  try {
...
// PubMed 医学文献搜索
export async function searchPubMed(query: string, limit: number = 5): Promise<ExternalSearchResult[]> {
  try {
    // 1. 搜索文章ID
    ...
    for (const id of ids) {
      ...
      if (article) {
        results.push({
          source: 'pubmed',
          title: article.title,
          content: `Authors: ${article.authors?.map((a: any) => a.name).join(', ') || 'Unknown'}`,
...
export async function multiSourceSearch(
  query: string,
  sources: string[] = ['wikipedia', 'arxiv', 'github', 'pubmed']
): Promise<ExternalSearchResult[]> {
  const searchFunctions: Record<string, () => Promise<ExternalSearchResult[]>> = {

...
// 百度学术搜索 (需要爬虫或第三方API)
export async function searchBaiduScholar(query: string, limit: number = 5): Promise<ExternalSearchResult[]> {
  try {
...
export async function searchGoogleScholar(query: string, limit: number = 5): Promise<ExternalSearchResult[]> {
  try {
    const apiKey = process.env.SERPAPI_KEY
...
export async function searchCrossRef(query: string, limit: number = 5): Promise<ExternalSearchResult[]> {
  try {
    ...
    return res.data.message.items.map((item: any) => ({
      source: 'crossref',
...
export async function searchSemanticScholar(query: string, limit: number = 5): Promise<ExternalSearchResult[]> {
  try {
...
export async function searchCore(query: string, limit: number = 5): Promise<ExternalSearchResult[]> {
  try {
...
// 中国知网 CNKI (需要机构访问或爬虫)
export async function searchCNKI(query: string, limit: number = 5): Promise<ExternalSearchResult[]> {
  // 知网没有公开API，需要:
  // 1. 机构VPN访问
  // 2. 购买API服务
  // 3. 爬虫（有法律风险）
...
// 万方数据库 (需要机构访问)
export async function searchWanfang(query: string, limit: number = 5): Promise<ExternalSearchResult[]> {
  // 万方同样没有公开API
...
// 知乎搜索 (需要爬虫)
export async function searchZhihu(query: string, limit: number = 5): Promise<ExternalSearchResult[]> {
  try {
...
// 哔哩哔哩搜索 (视频教程)
export async function searchBilibili(query: string, limit: number = 5): Promise<ExternalSearchResult[]> {
  try {
...
// 豆瓣读书搜索
export async function searchDoubanBooks(query: string, limit: number = 5): Promise<ExternalSearchResult[]> {
  try {

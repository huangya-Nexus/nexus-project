  ...
  try {
    ...
    const exportData = {
      ...
      edges: edges.map((edge: any) => ({
        sourceTitle: nodes.find((n: any) => n.id === edge.sourceId)?.title,
        targetTitle: nodes.find((n: any) => n.id === edge.targetId)?.title,
        type: edge.type,
        label: edge.label
      })).filter((e: any) => e.sourceTitle && e.targetTitle)
    }

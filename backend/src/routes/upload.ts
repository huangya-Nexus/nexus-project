...
// 上传文件到阿里云 OSS
async function uploadToOSS(ossClient: any, filePath: string, originalName: string): Promise<string | null> {
  if (!ossClient) {
    return null // 本地存储模式
  }
...
// 文件上传接口
router.post('/:graphId/upload', authMiddleware, upload.single('file'), async (req: AuthRequest, res) => {
  try {

...
// 创建分享链接
router.post('/:graphId/share', authMiddleware, async (req: AuthRequest, res) => {
  try {
    ...
    const days = parseInt(expiresIn as string)
...
// 复制分享的图谱到自己的账户
router.post('/token/:shareToken/clone', authMiddleware, async (req: AuthRequest, res) => {
  try {
...
// 获取我的分享列表
router.get('/my/shares', authMiddleware, async (req: AuthRequest, res) => {
  try {
    const userId = req.user?.userId as string

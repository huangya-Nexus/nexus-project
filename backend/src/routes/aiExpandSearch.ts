...
// 计算关联度分数
function calculateRelevanceScore(node: any, searchTerm: string): number {
  const term = searchTerm.toLowerCase()
  let score = 0
  
  // 标题完全匹配 +100
  if (node.title?.toLowerCase() === term) score += 100
  // 标题包含 +50
  else if (node.title?.toLowerCase().includes(term)) score += 50
  ...
  if (node.keywords) {
    try {
      ...
      if (keywords.some((k: string) => k.toLowerCase().includes(term))) {
        score += 30
      }
    } catch (e) {}
  }
...
// 生成综合知识网络
function generateComprehensiveNetwork(searchTerm: string, existingNodes: any[], limit: number) {
  // 根据关键词智能决定生成数量
...
// 智能模板（根据关键词生成不同数量）
function getSmartTemplate(searchTerm: string, count: number) {
...
function getNodeSuffix(index: number, category: string) {
...
// 生成综合定义
function generateComprehensiveDefinition(title: string, level: string, category: string, keyword: string) {
  return `${title}是${keyword}领域的重要组成部分，属于${level}级别内容。它涵盖了${category}方面的核心知识和实践方法，是深入理解${keyword}不可或缺的一环。掌握这个知识点需要系统学习和持续实践。`
}

// 生成综合内容
function generateComprehensiveContent(title: string, category: string, keyword: string) {
  return `# ${title}
...
// 生成综合属性
function generateComprehensiveAttributes(category: string, keyword: string) {
...
// 生成综合案例
function generateComprehensiveExamples(title: string, keyword: string) {
  return [
...
// 生成综合应用
function generateComprehensiveApplications(title: string, keyword: string) {
  return [
    `日常生活场景中的${title}应用`,
    `工作职场中的${title}实际运用`,
    `学术研究中的${title}辅助作用`,
...
// 生成综合资源
function generateComprehensiveResources(title: string, keyword: string) {
  return [
...
// 生成常见错误
function generateComprehensiveMistakes(title: string, keyword: string) {
  return [
    `忽视${title}的基础概念，急于求成`,
...
// 生成技巧
function generateComprehensiveTips(title: string, keyword: string) {
  return [
...
// 生成相关概念
function generateRelatedConcepts(title: string, keyword: string) {
  return [
    `${keyword}基础理论`,
    `${keyword}核心方法`,
    `${keyword}实践技巧`,
...
// 生成引用来源
function generateCitations(title: string, keyword: string) {
  return [
...
// 其他辅助函数...
function generateSmartDefinition(searchTerm: string, isTech: boolean, isAcademic: boolean, isSkill: boolean) {
  if (isTech) return `${searchTerm}是现代信息技术领域的核心技术，涵盖理论基础、算法实现、工程实践和创新应用。`
  if (isAcademic) return `${searchTerm}是人类知识体系中的重要学科，具有深厚的理论基础和广泛的应用价值。`
  if (isSkill) return `${searchTerm}是一项重要的软技能，对个人职业发展和人际交往具有关键作用。`
  return `${searchTerm}是一个重要的知识领域，涵盖理论基础、实践方法、应用技巧和发展趋势。`
}

function generateSmartBackground(searchTerm: string) {
  return `${searchTerm}经历了长期的发展和演变，从早期的理论探索到现代的广泛应用，不断有新的理论和实践成果涌现。`
}

function generateSmartImportance(searchTerm: string) {
  return `掌握${searchTerm}能够提升认知水平，增强问题解决能力，为个人发展开辟新的可能性，在现代社会具有重要意义。`
}

function generateScope(searchTerm: string) {
  return `${searchTerm}的研究范围包括基础理论、核心方法、实践应用、发展趋势等多个层面。`
}

function generateKeyFigures(searchTerm: string) {
  return ['领域奠基人', '重要贡献者', '现代领军人物', '活跃研究者']
}

function generateComprehensiveHistory(searchTerm: string) {
  return `${searchTerm}的发展经历了萌芽期、发展期、成熟期和繁荣期四个阶段，每个阶段都有重要的理论突破和技术创新。`
}

function generateComprehensiveTrends(searchTerm: string) {
  return `当前${searchTerm}呈现出技术迭代加速、应用场景拓展、跨学科融合、产业生态完善的发展趋势。`
}

function generateComprehensiveOutlook(searchTerm: string) {
  return `未来${searchTerm}将继续保持快速发展，技术创新将带来更多可能性，应用深化将创造更大价值。`
}

function generateGlobalPerspective(searchTerm: string) {
  return `${searchTerm}在全球范围内都有重要影响，不同国家和地区在该领域各有特色和优势。`
}

function generateComprehensiveExplanation(searchTerm: string) {
  return `${searchTerm}是一个多维度、多层次的知识体系，需要从理论、方法、应用等多个角度系统学习。`
}

function calculateAdvancedPosition(index: number, total: number) {
...
function generateComprehensiveEdges(nodes: any[], keyword: string) {
  const edges: any[] = []
...
function generateComprehensiveLearningPath(nodes: any[]) {
...
function groupIntoClusters(nodes: any[]) {
  ...
  nodes.forEach(node => {
    if (node.isPublic !== undefined) groups['已有知识'].push(node.id)
    else if (node.level === '基础') groups['基础知识'].push(node.id)
    else if (node.level === '进阶') groups['核心进阶'].push(node.id)
    else if (node.level === '高级') groups['高级应用'].push(node.id)
  })
...
function calculateDifficulty(nodes: any[]) {
  const weights = { '基础': 1, '进阶': 2, '高级': 3 }
  const total = nodes.filter(n => !n.isPublic).reduce((sum, n) => sum + (weights[n.level as keyof typeof weights] || 2), 0)
  const avg = total / nodes.filter(n => !n.isPublic).length || 1
  if (avg > 2.5) return '困难'
  if (avg > 1.8) return '中等'
  return '入门'
}

function calculateTotalTime(nodes: any[]) {
  const hours = nodes.filter(n => !n.isPublic).length * 4
  return `${hours}-${hours + 20}小时`
}

function calculateCompleteness(nodes: any[]) {
  const hasAllLevels = ['基础', '进阶', '高级'].every(level => 
    nodes.some(n => n.level === level)
  )

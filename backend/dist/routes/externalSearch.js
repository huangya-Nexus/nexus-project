...
import { searchWikipedia, searchArxiv, searchGitHub, searchPubMed, multiSourceSearch } from '../services/externalSearch.js';
const router = Router();
// 维基百科搜索
router.get('/wikipedia', async (req, res) => {
    try {
        ...
        const results = await searchWikipedia(q, parseInt(limit));
        res.json({
            source: 'wikipedia',
            query: q,
            results,
...
router.get('/arxiv', async (req, res) => {
    try {
        ...
        const results = await searchArxiv(q, parseInt(limit));
        res.json({
            source: 'arxiv',
            query: q,
            results,
...
router.get('/github', async (req, res) => {
    try {
        ...
        const results = await searchGitHub(q, parseInt(limit));
        res.json({
            source: 'github',
            query: q,
            results,
...
router.get('/pubmed', async (req, res) => {
    try {
        ...
        const results = await searchPubMed(q, parseInt(limit));
        res.json({
            source: 'pubmed',
            query: q,
            results,
...
router.get('/crossref', async (req, res) => {
    try {
        ...
        const results = await searchCrossRef(q, parseInt(limit));
        res.json({
            source: 'crossref',
            query: q,
            results,
...
router.get('/semantic_scholar', async (req, res) => {
    try {
        ...
        const results = await searchSemanticScholar(q, parseInt(limit));
        res.json({
...
router.get('/zhihu', async (req, res) => {
    try {
        ...
        const results = await searchZhihu(q, parseInt(limit));
        res.json({
            source: 'zhihu',
            query: q,
            results,
...
router.get('/bilibili', async (req, res) => {
    try {
        ...
        const results = await searchBilibili(q, parseInt(limit));
        res.json({
            source: 'bilibili',
            query: q,
            results,

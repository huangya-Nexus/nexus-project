...
const router = Router();
// 创建关联
router.post('/:graphId/edges', authMiddleware, async (req, res) => {
    try {
...
// 获取图谱的所有关联
router.get('/:graphId/edges', authMiddleware, async (req, res) => {
    try {
        const graphId = req.params.graphId;
        const userId = req.user.userId;
        // 验证图谱归属
...
// 删除关联
router.delete('/:graphId/edges/:edgeId', authMiddleware, async (req, res) => {
    try {
        const graphId = req.params.graphId;

...
const router = Router();
// SM-2 算法实现
function calculateNextReview(rating, // 1-4 (1=Again, 2=Hard, 3=Good, 4=Easy)
...
router.get('/today', authMiddleware, async (req, res) => {
    try {
        ...
        const uniqueCards = allCards.filter((card, index, self) => index === self.findIndex(c => c.id === card.id));
        res.json({
            cards: uniqueCards,
...
router.post('/:cardId/review', authMiddleware, async (req, res) => {
    try {
        ...
        // 计算下次复习时间
        const nextReview = calculateNextReview(rating, card.easeFactor, card.interval, card.repetitions);
        // 更新卡片
...
router.get('/stats', authMiddleware, async (req, res) => {
    try {
        ...
        const reviewedDates = new Set(reviewLogs.map(log => log.createdAt.toISOString().split('T')[0]));
